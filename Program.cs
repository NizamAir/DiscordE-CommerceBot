...
class Program
{
    ...
    {
        Console.WriteLine("Welcome to the Transcendental ECommerce Discord Online Bot " +
            "Advanced Console app, or TECDOBACA in short");
        Console.WriteLine("To start the bot press any key");
        Console.ReadKey();

        DiscordBot bot = new DiscordBot("botTokenId");
    ...
    {
        await channel.SendMessage(MessageTemplate.ProductList(products));
        ...
        while (true)
        {
            ...
            foreach (var message in messagesInTheChannel)
            {
                ...
                {
                    Console.WriteLine($"\nFound a price query, " +
                        $"the response from the bot is: ${message.Content} costs {price}");
                    string priceReport = MessageTemplate.PriceReport(message.Content, price);
                    await directMessage.Send(message.UserId, priceReport);
                    await channel.DeleteMessage(message.MessageId);
                }
                Console.WriteLine($"\nFound a message, the message is: {message.Content}");
            }
